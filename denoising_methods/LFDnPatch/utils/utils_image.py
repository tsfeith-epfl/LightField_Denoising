...
def snail_list(lst, group_neighbors=True):
    """
    Order a flattened matrix with snail ordering. If group_neighbors is True, then
    the 4-neighbors of each element are grouped together with it.
    The order for neighbors is [top, left, itself, right, bottom].
    If any neighbor is missing, it is replaced by the element itself.
    Example: [0_0, 0_1, 0_2, 1_0, 1_1, 1_2, 2_0, 2_1, 2_2], False -> [0_0, 0_1, 0_2, 1_2, 2_2, 2_1, 2_0, 1_0, 1_1]
    Example: [0_0, 0_1, 0_2, 1_0, 1_1, 1_2, 2_0, 2_1, 2_2], True -> [0_0_, 0_0, 0_0, 0_1, 1_0,
                                                                     0_1, 0_0, 0_1, 0_2, 1_1,
                                                                     0_2, 0_1, 0_2, 0_3, 1_2,
                                                                     ...
                                                                     ]
    """
    lst = sorted(lst)
    first_h, first_w = lst[0].split('.')[-2].split('_')[-2:]
    last_h, last_w = lst[-1].split('.')[-2].split('_')[-2:]
    ...
    matrix_copy = []
    for i in range(h):
        matrix.append(lst[i * w:(i + 1) * w])
        matrix_copy.append(lst[i * w:(i + 1) * w])

    # snail ordering
    while matrix:
        ordered_lst.extend(matrix.pop(0))
        if matrix and matrix[0]:
            for row in matrix:
                ordered_lst.append(row.pop())
        if matrix:
            ordered_lst.extend(matrix.pop()[::-1])
        if matrix and matrix[0]:
            for row in matrix[::-1]:
                ordered_lst.append(row.pop(0))

    if group_neighbors:
        output = []
        for img in ordered_lst:
            h_pos, w_pos = img.split('.')[-2].split('_')[-2:]
            h_pos, w_pos = int(h_pos) - first_h, int(w_pos) - first_w

            if h_pos == 0:
                top = img
            else:
                top = matrix_copy[h_pos - 1][w_pos]

            if h_pos == h - 1:
                bottom = img
            else:
                bottom = matrix_copy[h_pos + 1][w_pos]

            if w_pos == 0:
                left = img
            else:
                left = matrix_copy[h_pos][w_pos - 1]

            if w_pos == w - 1:
                right = img
            else:
                right = matrix_copy[h_pos][w_pos + 1]

            output += [top, left, img, right, bottom]
    else:
        output = ordered_lst
...
def row_list(lst, add_borders=True):
    """
    Return a list of image paths in row order.
    To prevent border effects, the first and last images per row are repeated
    if add_borders is True.
    """
    lst = sorted(lst)
    first_h, first_w = lst[0].split('.')[-2].split('_')[-2:]
    last_h, last_w = lst[-1].split('.')[-2].split('_')[-2:]
