...
def norm(lst: list) -> float:
    ...
    return np.sqrt(np.sum(np.array(lst) ** 2))
...
class Kernel(object):
    ...
    def __init__(self, size: tuple = (10, 10), intensity: float = 0.2):
    ...
    def _create_path(self):
        ...
        # second we find the angles of the motion blur steps
        def get_angles():
            """[summary]
    ...
    def kernel_matrix(self, *kargs):
        raise NotImplementedError("Can't manually set kernel matrix yet")

    def add_noise(self, image, keep_image_dim: bool = False) -> Image:
        """[summary]
        ...
        def apply2PIL(image: Image, keep_image_dim: bool = False) -> Image:
            """[summary]
        ...
        if isinstance(image, str) or isinstance(image, Path):
            ...
            return apply2PIL(image, keep_image_dim)
...
if __name__ == '__main__':
    ...
    k.add_noise(image, keep_image_dim=True).show()